...
//-------------------LUUVE-----------------------------/// 
void MANI(){ 
    ll n,m,k,ans=0,sum=0,ele=0,c=0;cin>>n;
    vector<ll>a(n),b(n),v;
    for(auto &i:a)cin>>i;
    for(auto &i:b)cin>>i;
    ll zo=0,oz=0,sma=0,smb=0;
    for(ll i=0;i<n;i++){
        if(a[i]!=b[i]){
            ele+=a[i];
            c++;
            if(a[i]==0 and b[i]==1)zo++;
            else oz++;
        }
        sma+=a[i];
        smb+=b[i];
    ...
    if(c==0)cout<<0;
    else if(sma==0 or smb==n){
        cout<<-1;
    }
    else if(c>0 and ele&1)cout<<1;
    else if(c>0 and ele%2==0)cout<<2;
}
