...
bool solve(string cur,string best){
    ll i=0,j=0;
    while(i<cur.size() and cur[i]=='0')i++;
    while(j<best.size() and best[j]=='0')j++;
    ll la=cur.size()-i,lb=best.size()-j;
    if(la!=lb)return la<lb;
    while(i<cur.size() and j<best.size()){
        if(cur[i]!=best[j])return cur[i]<best[j];
        i++,j++;
    }
    return false;
}
