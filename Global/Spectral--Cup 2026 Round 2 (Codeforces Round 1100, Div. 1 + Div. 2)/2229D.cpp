...
void MANI(){ 
    ...
    while(l<=r){
        ...
        vector<int>bin(n, -2);
         for(ll i = 0; i < n; i++){
            if(a[i] >= mid && b[i] >= mid)bin[i] = 1;
            else if(a[i] < mid && b[i] < mid)bin[i] = -1;
            else if(a[i] >= mid && b[i] < mid || (a[i] < mid && b[i] >= mid))bin[i] = 0;
        }
        ll zero = 0, pos = 0;
        for(ll i = 0; i < n; i++){
            if(bin[i] == 1)pos++;
            else if(bin[i] == 0)zero++;
            else if(bin[i] == -1) {
                pos--;
                int idx = i;
                while(idx + 1 < n && (bin[idx + 1] == -1 || bin[idx + 1] == 0))idx++;
                i = idx;
            }
        }
        if(pos + zero > 0 && pos > 0 || pos > 1)ans = mid, l = mid + 1;
        else r = mid - 1;
    }
    cout<<ans;
}
