...
bool possible(int tickets, int x, int y, ll a, ll b, ll k, vector<ll> &v, int n) {
 
	ll lcm = a*b/gcd((ll)a, (ll)b);	
	ll tot = 0;
	for (int i = 0; i < tickets/lcm && i < n; i++) {
		tot += ((v[i] / 100) * (x + y));
	}
	for (int i = tickets/lcm; i < tickets/a && i < n; i++) {
		tot += ((v[i] / 100) * x);
	}
	for (int i = tickets/a; i < -tickets/lcm + tickets/a + tickets/b && i < n; i++) {
		tot += ((v[i] / 100) * y);
	}
	return tot >= k;
