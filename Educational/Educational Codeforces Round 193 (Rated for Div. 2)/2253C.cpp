...
//-------------------LUUVE-----------------------------/// 
void MANI(){ 
    ll n,m,k,ans=0,sum=0,x,y;cin>>n>>m>>x>>y;
    vector<ll>a(x),b(y),v,com;
    for(auto &i:a)cin>>i;
    for(auto &i:b)cin>>i;
    vector<ll>ax,bx;
    ll i=0,j=0,reso=0;
    while(i<x or j<y){
        if(i<x and j<y and a[i]==b[j]){
            com.push_back(a[i]);
            i++,j++;
        }
        else if(j==y or (i<x and a[i]<b[j])){
            ax.push_back(a[i]);
            i++;
        }
        else{
    ...
    ll ax_size=ax.size();
    ll bx_size=bx.size();
    ll ta=min((ll)ax.size(),n);
    ll tb=min((ll)bx.size(),m);
    for(auto i:com)v.push_back(i);
    for(ll i=ax_size-ta;i<ax_size;i++){
        v.push_back(ax[i]);
    }
    for(ll i=bx_size-tb;i<bx_size;i++){
        v.push_back(bx[i]);
    }
    sort(v.rbegin(),v.rend());
    for(ll i=0;i<min(n+m-1,(ll)v.size());i++){
       ans+=v[i];
    }
    cout<<ans;
}
