...
//-------------------LUUVE-----------------------------/// 
void MANI(){ 
    ll n,m,k,ans=0,sum=0,res=0;cin>>n;
    vector<ll>a(n),b(n),v,sz;
    for(auto &i:a)cin>>i;
    ll maxi=0,c=1,pre=-1;
    for(int i=0;i<n;i++){
        if(v.empty() or a[i]!=v.back()){
            v.push_back(a[i]);
            sz.push_back(1);
        }
        else{
    ...
    for(ll i=0;i<m;i++){
        if(sz[i]>=2){
            bool f1=(i>0) and (i==1 or v[i-2]!=v[i]);
            bool f2=(i<m-1) and (i==m-2 or v[i]!=v[i+2]);
            if(f1 or f2){
                cout<<m+1;
                return;
            }
    ...
    //     return;
    // }
    // bool th=false,fr=false;
    // for(ll i=3;i<n;i++){
    //     if(a[i]==a[i-1] and a[i-2]==a[i-3] and a[i-1]!=a[i-2])fr=true;
    // }
    // for(ll i=3;i<n;i++){
    //     if(a[i]!=a[i-3] and a[i-2]!=a[i-3])th=true;
    // }
    // if(n>=3){
    //     if(a[0]!=a[1] and a[1]==a[2])th=true;
    //     if(a[n-1]!=a[n-2] and a[n-2]==a[n-3])th=true;
    // }
    // if(fr)cout<<c+2;
    // else if(th)cout<<c+1;
    // else cout<<c;
