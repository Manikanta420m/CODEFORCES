...
//-------------------LUUVE-----------------------------/// 
void MANI(){ 
    ll n,m,k,ans=1e18,sum=0,p,q;cin>>n>>k>>p>>q;
    vector<ll>a(n),b(n),v;
    for(auto &i:a)cin>>i;
    vector<ll>s1(n+1,0),s2(n+1,0),s3(n+1,0),mini(n+1,0);
    for(ll i=0;i<n;i++){
        s1[i+1]=s1[i]+a[i]%p;
        s2[i+1]=s2[i]+a[i]%q%p;
        s3[i+1]=s3[i]+a[i]%q;
        mini[i+1]=mini[i]+min(a[i]%p,min(a[i]%q%p,a[i]%q));
    }
    for(ll i=k-1;i<n;i++){
        ll res1=s1[i+1]-s1[i-k+1];
        ll res2=s2[i+1]-s2[i-k+1];
        ll res3=s3[i+1]-s3[i-k+1];
        ll res=min(res1,min(res2,res3));
        res+=mini[i-k+1]+mini[n]-mini[i+1];
        ans=min(ans,res);
    }
    cout<<ans;
}
