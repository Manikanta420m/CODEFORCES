...
void MANI(){ 
    ll n,m,k,ans=0,one=0,zero=0,chuk=0,sum=0;cin>>n>>m;
    vector<ll>a(n),b(n),v;
    for(auto &i:a)cin>>i;
    vector<ll>ones,zeros,chunks;
    ones.push_back(0);
    zeros.push_back(0);
    chunks.push_back(0);
    for(ll i=0;i<n;i++){
        ...
        if(i>0 and a[i]==a[i-1])chuk++;
        ones.push_back(one);
        zeros.push_back(zero);
        chunks.push_back(chuk);
    }
    while(m--){
        ll l,r;cin>>l>>r;
        if((r-l+1)%3!=0)cout<<-1<<endl;
        else if((ones[r]-ones[l-1])%3!=0)cout<<-1<<endl;
        else if((zeros[r]-zeros[l-1])%3!=0)cout<<-1<<endl;
        else{
            if(chunks[r]-chunks[l]>=1){
                cout<<(r-l+1)/3<<endl;
            }
            else{
                cout<<((r-l+1)/3)+1<<endl;
