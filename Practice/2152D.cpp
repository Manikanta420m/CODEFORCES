...
int get_first(ll n){ return 64-__builtin_clzll(n); }
...
void MANI(){ 
    ll n,m,k,ans=0,sum=0;cin>>n>>m;
    vector<ll>a(n+1),bls(n+1,0),fight(n+1,0);
    for(ll i=1;i<=n;i++){
        cin>>a[i];
        bls[i]=get_first(a[i])-1;
        if(a[i]&(a[i]-1)){
            if((a[i]-1)&(a[i]-2)){
                bls[i]++;
            }
    ...
    while(m--){
        ll l,r;cin>>l>>r;
        ll res=bls[r]-bls[l-1];
        ll f=fight[r]-fight[l-1];
        res+=f/2;
        cout<<res<<endl;
    }
}
