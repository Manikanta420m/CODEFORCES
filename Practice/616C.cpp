...
void MANI(){ 
    ...
    vector<vector<ll>>compId(n,vector<ll>(m,-1));
    for(ll i=0;i<n;i++){
        for(ll j=0;j<m;j++){
            char ch;cin>>ch;
            if(ch=='*'){
        ...
        for(ll j=0;j<m;j++){
            if(!a[i][j] and compId[i][j]==-1){
                    ...
                    while(!q.empty()){
                            ...
                            ll nx=dx[j]+x;
                            ll ny=dy[j]+y;
                                if(nx>=0 and ny>=0 and nx<a.size() and ny<a[0].size()  and !a[nx][ny] and compId[nx][ny]==-1){
                                    compId[nx][ny]=id;
                                    q.push({nx,ny});
                                }
                            }
